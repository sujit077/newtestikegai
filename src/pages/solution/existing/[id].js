...
function PizzaBar({step, setStep}){
    return(
        // #55BEFF
        <div className=" bg-gra top-[10rem] flex gap-x-3 items-center text-sm font-semibold select-none">
...
export default function ExistingSolution(){

    // Get the required getters and setters from Zustand state store
    const {agents, addAgent, setAgents, getAllAgents, setAgentToolIsUnfilled} = useAgentStore()
    const {setUseCase, useCase, generateId, getUseCaseIsFilled, setUseCaseIsFilled, setUseCaseName, setUseCaseFunc, setUseCaseDesc, setLLMName, setLLMType, setLLMVersion, setUseCaseAgents, setEvaluationMetrics, getUseCase, setDatasets, setGeneralPrompts, addGeneralPrompts, removeGeneralPrompts, getGeneralPrompts, setFeature, addFeature, removeFeature, getFeature, } = useCaseStore()
    const {getDataStores, newDataStore, getDataStore, getIngestionFiles} = useDataStore()

    // Set the local states of current UseCase, current stage of registration, LLM Types, current LLM, ingestion status & upload status
    ...
    // Setting states for comparisons of datasource
    ...
    useEffect(() => {
        ...
        // Get data for the current UseCase and fit it into the Zustand store
        ...
        .then((res)=>{
            ...
            setInitialDatasource(JSON.stringify(res.data.data.linked_datasources?.datasets) || "[]")

            setUseCase(res.data.data)

            if(res.data.data.config_manager?.agents?.length === 0 || res.data.data.config_manager?.agents?.length == undefined){
                addAgent()
                setUseCaseAgents(getAllAgents())

                // console.log(getAllAgents(), getUseCase());
            }
            else{
    ...
    function submitAddAgents(e){
        ...
        // console.log(getUseCase(), 'usecase data')
        ...
        let finalDatasource = JSON.stringify(getDataStores())

        // console.log(initialDatasource, finalDatasource);
        // console.log(initialDatasource === finalDatasource)
        console.log(initialDatasource === finalDatasource)
        console.log(initialDatasource)
        console.log(finalDatasource)
        
        if(initialDatasource === finalDatasource){
            setIngestionStatus(false)
            setStep(prev=>prev+1)
            return
        }
                ...
                {   
                                            ...
                                            getGeneralPrompts()?.map((prompt, idx)=>{
                                                return(
                                                    <div key={idx} className="flex gap-2 items-center">
                                                        ...
                                                        <MdOutlineControlPointDuplicate className="-mr-1 transform rotate-90 cursor-pointer hover:scale-110 active:scale-95 duration-100" onClick={()=>{addGeneralPrompts(idx,"up");}} size={18}/>

                                                        <MdOutlineControlPointDuplicate className="-mr-1 transform -rotate-90 cursor-pointer hover:scale-110 active:scale-95 duration-100" onClick={()=>{addGeneralPrompts(idx,"down");}} size={18}/>
                    ...
                    <form onSubmit={(e)=>{submitLLMSelection(e)}} className="mt-10 text-[#00338d] flex flex-col gap-y-10  w-[70rem]">
                        ...
                        <div className="flex gap-x-14 text-xs">
                            ...
                            <div className="flex flex-col gap-y-1 w-1/3">
                                ...
                                <div className="font-semibold flex gap-2 items-center">
                                    ...
                                    <Tooltip title={"Pretrained model served as open or commercial. Commercial models are proprietary and require a license fee based on usage, but often offer better performance. Open source models are free to use, but may have limited capabilities compared to commercial models."}>
                                        <span className=" text-base mt-[0.7px] cursor-pointer"><IoInformationCircleOutline/></span>
                                    </Tooltip>
                                </div>
                            ...
                            <div className="flex flex-col gap-y-1 w-1/3">
                                ...
                                <div className="font-semibold flex gap-2 items-center">
                                    ...
                                    <Tooltip title={"Foundational LLM model based on model source."}>
                                        <span className=" text-base mt-[0.7px] cursor-pointer"><IoInformationCircleOutline/></span>
                                    </Tooltip>
                                </div>
                            ...
                            <div className="flex flex-col gap-y-1 w-1/3">
                                ...
                                <div className="font-semibold flex gap-2 items-center">
                                    ...
                                    <Tooltip title={"Different model versions/variants may be trained on different data and may offer different capabilities, performance and features."}>
                                        <span className=" text-base mt-[0.7px] cursor-pointer"><IoInformationCircleOutline/></span>
                                    </Tooltip>
                                </div>
                        ...
                        <div className="flex justify-end items-center mt-[rem]">
                            <button  className="text-xs font-bold w-[12rem] text-[#b497ff] hover:bg-[#b497ff] hover:text-white border-2 border-[#b497ff] ease-in-out duration-200 rounded-full px-2 py-1 flex justify-center items-end">Save & Next <span className="mb-[0.05rem]"><FiChevronsRight/></span></button>
                        </div>
                    ...
                    <form onSubmit={(e)=>{submitAddAgents(e)}} className="mt-10 text-[#00338d] flex flex-col gap-y-10  w-[70rem]">
                        ...
                        <div className="flex flex-col gap-y-4">
                            {/* Heading */}
                            <div className="text-2xl font-bold">
                                <span className="text-black font-semibold">Let's add</span> Agents & Tools
                            </div>

                            {/* Description */}
                            <div className="text-xs w-2/3 text-black">
                                Agent is a doer that utilizes <span className="font-semibold">various tools</span> to execute your specific tasks.
                                You can add <span className="font-semibold">multiple agents</span> to accomplish different objectives.
...
{/* <label className="container w-[30rem]">
    <input name="f1score" type="checkbox"/>
    <span className="checkmark"></span>
    
    <div className="-mt-1">
        <div className="font-semibold text-base">F1-Score</div>
        <div id="f1scoreDesc" className="text-black">The F1-score computes the ratio of the number of shared words between the model generation and the ground truth.</div>
    </div>
